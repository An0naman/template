...
def manage_entry_types_page():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, singular_label, plural_label, description, note_types, is_primary, has_sensors, show_labels_section, show_end_dates, enabled_sensor_types, custom_chat_prompt, diagram_examples FROM EntryType ORDER BY singular_label")
