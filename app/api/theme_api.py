...
def generate_theme_css(settings=None):
    ...
    if dark_mode:
        ...
        html:not(:has(body[style*="background: transparent"])) body {{
            background-image:
                radial-gradient(ellipse 80% 60% at 15% 15%, rgba(var(--theme-primary-rgb), 0.18) 0%, transparent 55%),
                radial-gradient(ellipse 70% 50% at 85% 75%, rgba(var(--theme-primary-rgb), 0.12) 0%, transparent 50%),
