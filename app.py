...
def manage_entry_types_page():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, singular_label, plural_label, description, note_types, is_primary, show_labels_section FROM EntryType ORDER BY singular_label")
    entry_types = cursor.fetchall()
